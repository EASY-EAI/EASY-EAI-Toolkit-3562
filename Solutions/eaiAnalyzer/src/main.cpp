...
#define PROCESS_ANALYZER_NAME   "analyzer"
...
int32_t main(int32_t argc, char **pcArg)
{
	...
	if(0 == strcmp(pcArg[1], "Main"))
	{
        ...
        /* 创建图像分析器 */
        CreateProcess(PROCESS_ANALYZER_NAME, &st_TaskInfo);
        
    
    /* 图像分析器子程进入此分支 */
    } else if(0 == strcmp(pcArg[1], PROCESS_ANALYZER_NAME)) {
        char cmd[128] = {0};
        sprintf(cmd,"mkdir -p %s", APP_PATH);
        exec_cmd_by_system(cmd);
        
        analyzerInit(PROCESS_ANALYZER_NAME);
    }
	...
	while(1)
	{
		...
		if(-1 == waitpid)
		{
			PRINT_ERROR("errno = %d,%s", errno, strerror(errno));
		}
		...
		for(byTaskNum = 0; byTaskNum < st_TaskInfo.dwTaskNum; byTaskNum++)
		{
			if(st_TaskInfo.st_task[byTaskNum].dwTaskID == waitpid)
			{
				...
				for (int i = 0; i < TASK_RESPAWN_RETRY_MAX_TIMES; i++)
				{
					...
					if (sdwRet == -1)
					{
						PRINT_ERROR("CreateProcess failed, retry index: %d", i);
						osTask_msDelay(500);
					}
					else
					{
						PRINT_DEBUG("task \"%s\" respawned", st_TaskInfo.st_task[byTaskNum].byTaskName);
						bSuccess = true;
						break;
					}
				}
				if (!bSuccess)
				{
					PRINT_ERROR("Failed to respawn child process \"%s\"",st_TaskInfo.st_task[byTaskNum].byTaskName);
				}
				break;
			}
		}
